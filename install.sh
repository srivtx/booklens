#!/bin/sh
# install.sh — install booklens from GitHub.
#
#   curl -fsSL https://raw.githubusercontent.com/srivtx/booklens/main/install.sh | sh
#
# Installs the `booklens` binary globally with Bun. Pass a git ref as the
# first argument to pin a branch, tag, or commit:
#
#   curl -fsSL https://raw.githubusercontent.com/srivtx/booklens/main/install.sh | sh -s v0.1.0
set -eu

REPO="srivtx/booklens"
BIN="booklens"
REF="${1:-main}"
